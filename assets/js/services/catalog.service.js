const CatalogService = (() => {
    ...
    let _dockerOptions = null;

    async function getBaremetalBundles() {
        try {
            return await Api.get('/api/catalog/bundles?deployTarget=baremetal');
        } catch {
    ...
    async function getDockerOptions() {
        if (_dockerOptions) return _dockerOptions;
        try {
            _dockerOptions = await Api.get('/api/catalog/modloader-options?deployTarget=docker');
        } catch {
            _dockerOptions = [
        ...
        return _dockerOptions;
    }

    function getUniqueVersions(options) {
        return [...new Set(options.map(o => o.minecraftVersion))].sort((a, b) => b.localeCompare(a, undefined, { numeric: true }));
    }

    function getLoadersForVersion(options, version) {
        return [...new Set(options.filter(o => o.minecraftVersion === version).map(o => o.modLoader))];
    }

    function getLoaderVersions(options, version, loader) {
        return options.filter(o => o.minecraftVersion === version && o.modLoader === loader).map(o => o.modLoaderVersion);
    }

    return { getBaremetalBundles, getDockerOptions, getUniqueVersions, getLoadersForVersion, getLoaderVersions };
})();
